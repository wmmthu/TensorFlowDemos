...
# discriminator
D_W = tf.Variable(xavier_init([p+c,n_hidden]))
...
# generator
G_W = tf.Variable(xavier_init([n_z+c,n_hidden]))
...
sampled_x = generator(z,y)

prob_real,logit_real = discriminator(x,y)
prob_fake,logit_fake = discriminator(sampled_x,y)
...
with tf.Session() as sess:
	...
	for loop in xrange(100000):
		...
		_,_D_loss = sess.run([D_train,D_loss],feed_dict={y:Y,x:X,z:np.random.uniform(-1,1,size=[64,n_z])})
		_,_G_loss = sess.run([G_train,G_loss],feed_dict={y:Y,z:np.random.uniform(-1,1,size=[64,n_z])})
		...
		if loop % 1000 == 0:
			...
			samples = sess.run(sampled_x,feed_dict={y:Y,z:np.random.uniform(-1,1,size=[16,n_z])})
			fig = plot(samples)
